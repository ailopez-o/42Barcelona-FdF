...
/*   Updated: 2022/06/29 16:01:17 by ailopez-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "../inc/fdf.h"
...
int limits(t_point  *points, int len)
{
    ...
    while (i < len)
    {
        if (points[i].axis[x] < FIT_MARGIN || points[i].axis[x] > (WINX - FIT_MARGIN))
            return (1);
        if (points[i].axis[y] < FIT_MARGIN || points[i].axis[y] > (WINY - FIT_MARGIN))
            return (1);
        i++;    
    }
    return(0);
